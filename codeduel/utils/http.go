...
func HttpGet(uri string, headers map[string]string, responseBody interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("failed to create GET(%s) request: %w", uri, err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to make GET(%s) request: %w", uri, err)
	}
...
func HttpPost(uri string, headers map[string]string, requestBody interface{}, responseBody interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("failed to create POST(%s) request: %w", uri, err)
	}
	...
	res, err := http.DefaultClient.Do(req)
	if err != nil {
		return fmt.Errorf("failed to make POST(%s) request: %w", uri, err)
	}
	defer res.Body.Close()
...
func HttpPatch(uri string, headers map[string]string, requestBody interface{}, responseBody interface{}) error {
	// Convert request body to JSON
	requestBodyBytes, err := json.Marshal(requestBody)
	if err != nil {
		return fmt.Errorf("failed to marshal request body: %w", err)
	...
	// Create request
	req, err := http.NewRequest("PATCH", uri, bytes.NewBuffer(requestBodyBytes))
	if err != nil {
		return fmt.Errorf("failed to create POST(%s) request: %w", uri, err)
	}
